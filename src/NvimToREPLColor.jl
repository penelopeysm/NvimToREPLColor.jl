...
const FACE_SPECS = FaceSpec[
    ...
    # Not a treesitter group. Handled specially in `build_error_face`, since
    # Julia's built-in julia_error has a red background that a faces.toml entry
    # can only override, never remove.
    FaceSpec("julia_error"; groups=["Error"]),
]

# The theme's base background, used to neutralise julia_error's default red
const NORMAL_GROUP = "Normal"
...
# Julia's default julia_error face is a solid red background, and
# julia_unpaired_parentheses inherits it. Faces loaded from faces.toml are
# merged onto the defaults (including the inherit list), so there is no way to
# switch the background off; the best we can do is paint it with the theme's
# Normal background so it disappears in a matching terminal, and signal the
# error with the theme's Error color plus a curly underline instead.
function build_error_face(hls::Dict{String,Hl})
    face = Face()
    err = get(hls, "Error", nothing)
    normal = get(hls, NORMAL_GROUP, nothing)
    color = err === nothing ? nothing : something(err.fg, err.bg, Some(nothing))
    color === nothing || push!(face, "foreground" => color)
    if normal !== nothing && normal.bg !== nothing
        push!(face, "background" => normal.bg)
    end
    push!(face, "underline" => color === nothing ? true : [color, "curly"])
    return face
end
...
toml_value(v::Vector) = "[" * join(toml_value.(v), ", ") * "]"  # e.g. underline = [color, style]
...
function run_cli(args)
    ...
    groups = unique(vcat([g for s in FACE_SPECS for g in s.groups],
        RAINBOW_FALLBACK_GROUPS, RAINBOW_PLUGIN_GROUPS, [NORMAL_GROUP]))
    hls = query_nvim(groups; colorscheme=opts["colorscheme"])
    colorscheme = something(opts["colorscheme"], nvim_colorscheme_name())

    faces = Pair{String,Face}[s.name => (s.name == "julia_error" ? build_error_face(hls) : build_face(s, hls))
                              for s in FACE_SPECS]
